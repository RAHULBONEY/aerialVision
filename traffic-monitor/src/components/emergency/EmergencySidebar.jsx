...
export default function EmergencySidebar() {
    ...
    const sidebarBg = theme === 'dark'
        ? 'bg-[#0a0a0f] border-r border-rose-900/30'
        : 'bg-white border-r border-slate-200';

    const logoBg = theme === 'dark'
        ? 'bg-gradient-to-br from-rose-600 to-red-700 shadow-[0_0_15px_rgba(225,29,72,0.4)]'
        : 'bg-gradient-to-br from-rose-500 to-red-600';

    const logoTextColor = theme === 'dark' ? 'text-rose-50' : 'text-slate-900';
    const sidebarTextColor = theme === 'dark' ? 'text-slate-400' : 'text-slate-600';
    const hoverBg = theme === 'dark' ? 'hover:bg-rose-900/20' : 'hover:bg-rose-50';

    const activeClass = theme === 'dark'
        ? 'bg-rose-900/40 border-l-2 border-rose-400 text-rose-400 shadow-[0_0_10px_rgba(251,113,133,0.1)]'
        : 'bg-rose-50 border-l-2 border-rose-600 text-rose-700';
    ...
    return (
        ...
        >
            {/* Logo */}
            <div className={`p-6 border-b ${theme === 'dark' ? 'border-rose-900/30' : 'border-slate-200'}`}>
            ...
            {/* Footer */}
            <div className={`p-4 border-t ${theme === 'dark' ? 'border-rose-900/30' : 'border-slate-200'} space-y-2`}>
                {/* User Info */}
                <div className={cn("flex items-center gap-3 px-2 py-2 mb-2 rounded-lg bg-opacity-50",
                    theme === 'dark' ? "bg-[#0f0a15]" : "bg-slate-50",
                    !isExpanded && "justify-center"
                )}>
                    <div className={`w-8 h-8 rounded-lg ${theme === 'dark' ? 'bg-rose-900/30' : 'bg-rose-100'} flex items-center justify-center border border-rose-500/20`}>
                        <User className={`w-4 h-4 ${theme === 'dark' ? 'text-rose-400' : 'text-rose-600'}`} />
                    </div>
                    {isExpanded && (
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-1.5">
                                ...
                                <p className="text-[10px] text-emerald-500 font-mono uppercase">On Duty</p>
                            </div>
                        </div>
                    )}
                </div>
                ...
                <button
                    ...
                    className={cn(
                        ...
                        theme === 'dark' ? 'hover:bg-rose-900/20 hover:border-rose-800/50 text-slate-400' : 'hover:bg-slate-100 text-slate-600',
                        !isExpanded && "justify-center"
                    )}
                >
